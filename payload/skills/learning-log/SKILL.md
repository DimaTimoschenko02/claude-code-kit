...
1. Загрузить open mistakes (как в `/learning-log`), помня для каждой её **файл-день** и блок.
...
4. Для каждой группы/записи — **сверить с registry** (семантика + Related + Cause): новый паттерн или рецидив уже разобранного?
   ...
   - **Runtime-fix** у записи (в моменте выкрутился) → разбираем **корень**; тривиально/разово → `wontfix` «awareness only».
