...
- Разобранная запись флипается **на месте** (`Status` + `Resolution`/`Promoted-to`); `/learning-log` фильтрует по `Status: open`.
- **Периодический компакт (решение Димы 2026-07-04):** после крупного `analyze`/`wins`-разбора гони `python3 .claude/hooks/learning-log-compact.py` — удаляет закрытые (addressed/wontfix) блоки и опустевшие дневные файлы; open не трогает. Durable-след паттерна = строка в `resolutions.md` (Recur-счётчик), детали события после разбора не нужны.
...
0. **Частотный скан ПЕРЕД чтением записей** (добавлено 2026-08-01). LL — микроскоп: он видит
   событие и слеп к частоте, к трениям среды и к неэффективности. Эти два скрипта дают
   вторую половину картины, детерминированно и за секунды:
   ```bash
   # если в проекте есть частотные анализаторы транскриптов — гнать их здесь, напр.:
   python3 .claude/scripts/transcript-stats.py --days <N> --top 30   # что зову и пишу заново
   python3 .claude/scripts/hook-friction.py   --days <N>             # что блокируют хуки/классификатор
   ```
   Скриптов нет — не выдумывать замену, а просто отметить, что частотная картина не снята.
   Читать ДО разбора: они переопределяют приоритет. Прецедент 2026-08-01 — топ-2 трения
   (инлайн-парсер написан ~167 раз, 87 промахов путём из-за сброса cwd) в логе отсутствовали
   ПОЛНОСТЬЮ, а «главная skill-дыра» из ощущений оказалась мифом: реальная дыра была одна (Jira).
   ⚠️ **Скан идёт по ВСЕМ папкам транскриптов** (`~/.claude/projects/*/`), не по одной
   `-Users-dimti-work` — дефект 8.4, пойман дважды: сначала в скилле `daily`, потом на
   агентах-майнерах, которые прочли 1 папку из 21 и выдали это за полную картину.
